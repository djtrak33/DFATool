...
void BlockLocalPositionEstimator::gpsInit()
{
	...
	if (_gpsStats.getCount() > REQ_GPS_INIT_COUNT) {
		...
		//PX4_INFO("Lat1 value in estimator is: %f", gpsLat);
		//PX4_INFO("Lon1 value in estimator is: %f", gpsLon);
...
void BlockLocalPositionEstimator::gpsCorrect()
{
	...
	//PX4_INFO("Lat value in estimator is: %f", lat);
	//PX4_INFO("Lon value in estimator is: %f", lon);
	float px = 0;
	float py = 0;
